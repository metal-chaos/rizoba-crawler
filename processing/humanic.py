...
class Humanic:
    ...
    def place(self):
        return re.search(r"<dt class=\"item_info_term\">\s*勤務地\s*<\/dt>\s*<dd class=\"item_info_description\">\s*(.*)\s*<\/dd>", self.soup)[1]
    ...
    def salary(self, request):
        ...
        salary = re.search(r"<dt class=\"item_info_term\">\s*(時給|日給|月給)\s*<\/dt>\s*<dd class=\"item_info_description\">\s*(.*)(円)\s*<\/dd>", self.soup)
    ...
    def picture(self):
        return re.search(r"<span class=\"item_slide_image\">\s*<img alt=\"[\s\S]*?\" src=\"([\s\S]*?)\"", self.soup)[1]

    def time(self):
        return re.search(r"<dd class=\"item_info_description work_time_unit\">\s*([\s\S]*?)\s*<\/dd>", self.soup)[1]

    def treatment(self):
        return re.search(r"<dt class=\"item_info_term\">\s*福利厚生\s*<\/dt>\s*<dd class=\"item_info_description\">\s*([\s\S]*?)\s*<\/dd>", self.soup)[1]

    def jobDesc(self):
        return re.search(r"<dt class=\"item_info_lead_term\">\s*仕事内容\s*<\/dt>\s*<dd class=\"item_info_lead_description\">\s*([\s\S]*?)\s*<\/dd>", self.soup)[1]

    def permaLink(self):
        return "detail-humanic-" + str(self.__urlNum[1])

    def meal(self):
        return "TRUE" if ("食費無料" in self.soup) else "FALSE"

    def wifi(self):
        return "TRUE" if ("食費無料" in self.soup) else "FALSE"

    def spa(self):
        return "TRUE" if ("item_merit skin_merit_m2_16" in self.soup) else "FALSE"

    def transportationFee(self):
        return "TRUE" if ("交通費支給" in self.soup) else "FALSE"
